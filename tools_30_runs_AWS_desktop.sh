...
do
	...
	do
		...
		echo "deepcrawl"
		timeout 300 node "crawler"$d".js" --site $domain --count 1 --headless --output-logs $path1$trial'_run'$run'_deepcrawl_top'$rank'.json' --port $port --output-cookies 'cookies'$rank'.json' 
		...
		echo 'zbrowse'
		timeout 300 node index.js $prefix$domain $port > $path2$trial'_run'$run'_zbrowse_top'$rank'.json'
		cd ../../
	...
	if (("$d" != 4)); then
		...
		do
			...
			cd zbrowse-master/js
			echo 'zbrowse'
			timeout 180 node index.js $prefix$domain $port > $path2$trial'_run'$run'_zbrowse_top'$rank'.json'
			cd ../../
