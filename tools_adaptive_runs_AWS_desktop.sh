...
do
	...
	do
		...
		timeout 260 ./tools_dp_zb_desktop.sh $trial $port $run $rank $domain $d $prefix
		#echo "Check if data empty"
		path=$path3$trial'_run'$run'_deepcrawl_top'
		dp=$(python parse_deepcrawl_output.py './desktop_deepcrawl_results/trial'$trial'_run'$run'_deepcrawl_inclusions_top'$rank'.json' $path$rank'_alldetails.txt' $path$rank'_alledges.txt' $path$rank'_allresources.txt' $path$rank'_alldomains.txt')
		echo 'parse_deepcrawl' $dp
		f=$path3$trial'_run'$run'_zbrowse_top'$rank
		zb=$(python parse_zbrowse_output.py './desktop_zbrowse_results/trial'$trial'_run'$run'_zbrowse_top'$rank'.json' $f'_alldetails.txt' $f'_alledges.txt' $f'_allresources.txt' $f'_alldomains.txt')
	...
	if (("$d" != 4)); then
		...
		timeout 130 ./data_collection_desktop.sh $trial $port $run $rank $domain $d $prefix

		echo "Finding diff"
		n=$(./diff_betn_runs.o $rank $run $trial ./desktop_tools_results/)
